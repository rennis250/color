...
source('./code/functions/robsblobs.R')

wd <- './data/'

# Load in Data ------------------------------------------------------
# d_form <- read.csv(paste(wd, 'gray_complete_data_release.csv', sep = ''))
# d_form$Ls_square <- d_form$Ls
# d_form$Lt1_square <- d_form$Lt1
# d_form$Lt2_square <- d_form$Lt2
# for (lc in 1:length(d_form$Ls)) {
#   rgb <- lab2rgb(c(d_form$Ls[lc], 0, 0))
#   rgb_square <- rgb^2
#   d_form$Ls_square[lc] <- rgb2lab(rgb_square)[1]
# 
#   rgb <- lab2rgb(c(d_form$Lt1[lc], 0, 0))
#   rgb_square <- rgb^2
#   d_form$Lt1_square[lc] <- rgb2lab(rgb_square)[1]
# 
#   rgb <- lab2rgb(c(d_form$Lt2[lc], 0, 0))
#   rgb_square <- rgb^2
#   d_form$Lt2_square[lc] <- rgb2lab(rgb_square)[1]
# }
# d_form$Ls <- d_form$Ls_square
# d_form$Lt1 <- d_form$Lt1_square
# d_form$Lt2 <- d_form$Lt2_square
# write.csv(d_form, file = paste(wd, 'rob_square.csv', sep = ''))
d_form <- read.csv(paste(wd, 'rob_square.csv', sep = ''))
d_form <- subset(d_form, select = -c(Ls_square, Lt1_square, Lt2_square))
d_form$Ls <- round(d_form$Ls)
d_form$Lt1 <- round(d_form$Lt1)
d_form$Lt2 <- round(d_form$Lt2)
