...
# d_form$Ls_sqrt <- d_form$Ls
# d_form$Lt1_sqrt <- d_form$Lt1
# d_form$Lt2_sqrt <- d_form$Lt2
# for (lc in 1:length(d_form$Ls)) {
#   rgb <- lab2rgb(c(d_form$Ls[lc], 0, 0))
#   rgb_sq <- sqrt(rgb)
#   d_form$Ls_sqrt[lc] <- rgb2lab(rgb_sq)[1]
#   
#   rgb <- lab2rgb(c(d_form$Lt1[lc], 0, 0))
#   rgb_sq <- sqrt(rgb)
#   d_form$Lt1_sqrt[lc] <- rgb2lab(rgb_sq)[1]
#   
#   rgb <- lab2rgb(c(d_form$Lt2[lc], 0, 0))
#   rgb_sq <- sqrt(rgb)
#   d_form$Lt2_sqrt[lc] <- rgb2lab(rgb_sq)[1]
# }
# d_form$Ls <- d_form$Ls_sqrt
# d_form$Lt1 <- d_form$Lt1_sqrt
# d_form$Lt2 <- d_form$Lt2_sqrt
# write.csv(d_form, file = paste(wd, 'rob_sqrt.csv', sep = ''))
d_form <- read.csv(paste(wd, 'rob_sqrt.csv', sep = ''))
d_form <- subset(d_form, select = -c(Ls_sqrt, Lt1_sqrt, Lt2_sqrt))
